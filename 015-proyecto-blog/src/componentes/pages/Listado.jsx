import React from "react";
import {Global} from "../../helpers/Global";
import {Peticion} from "../../helpers/Peticion";
import {Link} from "react-router-dom";

//Componentes con propiedades
export const Listado = ({articulos, setArticulos}) => {
	const eliminar = async(id) => {
		//alert(id);
		let {datos} = await Peticion(Global.url+"articulo/"+id,"DELETE");
		//console.log(datos);
		if(datos.status == "success"){
			let articulosUp = articulos.filter(art => art._id !== id);
			setArticulos(articulosUp);
		}
	}

	return (
		articulos.map(art => {//Recorrer articulos
						return(
							 <article key={art._id} className="articulo-item">
						 		<div className="mascara">
						 			{//si la imagen es diferente a default.png
						 				art.imagen != "default.png" && <img src={Global.url+ "imagen/"+art.imagen} />
						 			}
						 			{//Si la imagen es igual a default.png
						 				art.imagen == "default.png" && <img src="data:image/png;base64," />
						 			}
						 			

						 		</div>
						 		<div className="datos">
							 		<h3 className="title">
					                    <Link to={"/articulo/"+art._id}>{art.titulo}</Link>
					                    </h3>
					                    <p className="description">
					                        {art.contenido}
					                    </p>

					                    <Link to={"/editar/"+art._id} className="edit">Editar</Link>
					                    <button className="delete" onClick={() => {
					                    	eliminar(art._id)
					                    }}>Borrar</button>
					                
						 		</div>   
				            </article>
						)
					})
	)
}